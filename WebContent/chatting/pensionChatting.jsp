...
<html>
            ...
            <tr>
               ...
               <td>
                  <div class="input-group">
                          <label for="inputId" id="a"><div id="memChat">${sessionScope.memId}</div></label>
                       </div>
                    </td>
   ...
   <script type="text/javascript">
      ...
      function openserver() {
         ...
         if(!ws) {
            ...
            ws.onmessage = function(evt) {
               ...
               if (user[1] == $('#memChat').html()) {
                  if (user[0] != $('#memChat').html()) {
                     msg.innerHTML += "관리자 : ";
                     for(var i = 2; i < user.length; i ++ ) {
                        msg.innerHTML += user[i];
                     }
                  } else if (user[0] == $('#memChat').html()) {
                     msg.innerHTML += $('#memChat').html() + " : ";
                     for(var i = 2; i < user.length; i ++ ) {
                        msg.innerHTML += user[i];
                     }
                  } else {
      ...
      function sendmsg() {
         var result = document.getElementById("result");
         var msg = $('#memChat').html() + " : " + $('#memChat').html() + " : " + inputform.message.value;
         ws.send(msg);
