...
<%@ taglib prefix="sql" uri="http://java.sun.com/jsp/jstl/sql" %>    
	...
	<sql:query var="rsdate" dataSource="jdbc/kh">
		select distinct to_char(b_reg_date,'yyyy-mm-dd') b_reg_date 
		from pension_book where b_reg_date >= ? order by b_reg_date asc
	...
	<books>
		<data>
			{
				"book" : [
					<c:forEach var="date" items="${rsdate.rows}">	
						...
						<c:forEach var="r" begin="1" end="5">
							...
							<sql:query var="rs" dataSource="jdbc/kh">
								...
								<sql:param value="${r}" />
								<sql:param value="${date.b_reg_date}" />
							</sql:query>
